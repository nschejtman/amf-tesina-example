...
class PelletTest extends AnyFunSuite {
  ...
  test("PII Case Study simple example") {
    ...
    val dataInputStream = new FileInputStream("src/test/resources/test/cases/pii/simple-transfer/simple.ttl")
    data.read(dataInputStream, null, "TTL")

    // Read ontology
    val ontology            = ModelFactory.createOntologyModel()
    val ontologyInputStream = new FileInputStream("src/test/resources/test/cases/pii/PII.ontology.ttl")
    ...
    val query          = QueryFactory.read("src/test/resources/test/cases/pii/list-pii-sensitive.sparql")
